  ...
  Future<void> _uploadCableSchedule() async {
    ...
    try {
      ...
      if (result != null && result.files.isNotEmpty) {
        ...
        print(
          'Selected file: ${platformFile.name}, Size: ${platformFile.size} bytes',
        );
        ...
        if (!['csv', 'xlsx', 'xls'].contains(fileExtension)) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text(
                  'Please select a CSV or Excel (.xlsx/.xls) file.',
                ),
              ),
            );
          }
          ...
          try {
            ...
            if (mounted) {
              ScaffoldMessenger.of(context).showSnackBar(
                const SnackBar(
                  content: Text(
                    'Excel file conversion not supported. Please convert to CSV first.',
                  ),
                ),
              );
            }
        ...
        if (csvTable.isEmpty) {
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(
                content: Text('Selected file is empty or could not be parsed.'),
              ),
            );
          }
          setState(() {
        ...
        print(
          'Column indices - SCB: $scbNoIndex, ICR: $icrNoIndex, Inverter: $inverterNoIndex, Length: $scheduledLengthIndex',
        );
        ...
        for (int i = 1; i < csvTable.length; i++) {
          ...
          if (row.length > scheduledLengthIndex) {
            try {
              ...
              final String inverterNo = row[inverterNoIndex]?.toString() ?? '';
